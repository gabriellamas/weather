  ...
  return !geolocation.error && !geolocation.browserError && geolocation.value ? (
    <Container>
      <Content container spacing={2}>
        {daily ? daily.filter(item => item.date.diff(dy(), 'days') < 6).map(day => (
          <Grid item key={day.date.format()}>
            <DailyWeatherCard
              day={day.date.locale('pt-br').calendar(null, {
                sameDay: '[Hoje]',
                nextDay: '[Amanhã]',
                nextWeek: 'dddd',
                sameElse: 'DD [de] MMMM'
